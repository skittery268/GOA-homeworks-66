...
const PostCard = ({ post, feed=false, setIsEditing, setEditPost }) => {
    const { user } = useAuth();
    ...
    return (
        <div className="glass-card p-6">
            <div className="flex items-start gap-3">
                <Link to={!feed ? "/dashboard" : `/profile/${post.user?._id}`} className="w-10 h-10 rounded-full bg-gradient-to-br from-coral to-gold flex items-center justify-center shrink-0">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                        <Link to={!feed ? "/dashboard" : `/profile/${post.user?._id}`} className="text-sm font-semibold text-ink">
